...
def seed():
    db = SessionLocal()
    try:
        # 清空旧数据（使用 DELETE 而非 TRUNCATE，兼容 SQLite 和 MySQL）
        db.execute(text("DELETE FROM fnos_trades"))
        db.execute(text("DELETE FROM fnos_positions"))
        db.execute(text("DELETE FROM fnos_serial_counters"))
        db.commit()
