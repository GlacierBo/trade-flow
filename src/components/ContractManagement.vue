...
function save() {
  ...
  saving.value = true
  if (adding.value) {
    store.updateContract(editCode.value, codeInput.value.trim(), nameInput.value.trim())
  } else {
    store.addContract(codeInput.value.trim(), nameInput.value.trim())
  }
  saving.value = false
...
<template>
  <div>
    ...
    <div class="bg-gray-800/30 border border-gray-700/30 rounded-2xl overflow-hidden">
      <div v-if="!store.contracts.length" class="flex flex-col items-center justify-center py-16 text-gray-500">
    ...
    <div v-if="showForm" class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div class="bg-gray-800 border border-gray-700 rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl animate-fadeIn">
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-700/50">
          <h3 class="text-base font-black text-gray-100">{{ adding ? '编辑合约' : '新增合约' }}</h3>
        ...
        <div class="flex gap-3 px-5 pb-5">
          ...
          <button class="flex-1 py-2.5 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 rounded-xl text-white font-bold text-sm transition-all" :disabled="!codeInput.trim() || !nameInput.trim()" :class="(!codeInput.trim() || !nameInput.trim()) ? 'opacity-50' : ''" @click="save">确定</button>
        </div>
      </div>
    </div>
  </div>
